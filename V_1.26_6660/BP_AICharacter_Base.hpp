...
class ABP_AICharacter_Base_C : public ABP_TPSCharacter_C
{
    ...
    class UDespawnComponent* DespawnComponent;                                        // 0x1E20 (size: 0x8)
    class UBPStatsComponent_C* BPStatsComponent;                                      // 0x1E28 (size: 0x8)
    class UOptionListenerComponent* OptionListener;                                   // 0x1E30 (size: 0x8)
    class UFightingAIDialogComponent* FightingAIDialog;                               // 0x1E38 (size: 0x8)
    class UASMAIComponent* ASMAI;                                                     // 0x1E40 (size: 0x8)
    class UAIFightingCameraWeightComponent* AIFightingCameraWeight;                   // 0x1E48 (size: 0x8)
    class UAIFightingComponent* AIFighting;                                           // 0x1E50 (size: 0x8)
    FString FoleySwitch;                                                              // 0x1E58 (size: 0x10)
    bool PlayFightDialogLine;                                                         // 0x1E68 (size: 0x1)
    bool WasSurprised?;                                                               // 0x1E69 (size: 0x1)
    FBP_AICharacter_Base_CPlayerDetected PlayerDetected;                              // 0x1E70 (size: 0x10)
    void PlayerDetected();
    FString Character Voice;                                                          // 0x1E80 (size: 0x10)
    class UParticleSystemComponent* PS_ChargeOwner_Instance;                          // 0x1E90 (size: 0x8)
    class UStaticMeshComponent* Accesory;                                             // 0x1E98 (size: 0x8)
    FDataTableRowHandle Accessory;                                                    // 0x1EA0 (size: 0x10)
    class UMaterialInstanceDynamic* HideScalpMaterial;                                // 0x1EB0 (size: 0x8)
    class ABP_DynamicAccessory_Master_C* Ejecting Prop;                               // 0x1EB8 (size: 0x8)
    int32 Ejected Prop Index;                                                         // 0x1EC0 (size: 0x4)
    float TargetLocationZOffsetInDown;                                                // 0x1EC4 (size: 0x4)
    FGameplayTag OnDeathDetectiveTag;                                                 // 0x1EC8 (size: 0x8)
    FText OnDeathDetectiveText;                                                       // 0x1ED0 (size: 0x18)
    class UParticleSystemComponent* LastManAuraEmitter;                               // 0x1EE8 (size: 0x8)
    FName BossDialogKey;                                                              // 0x1EF0 (size: 0x8)
    TArray<class UParticleSystemComponent*> NewVar_0;                                 // 0x1EF8 (size: 0x10)
    class UParticleSystemComponent* PS Charge Aura Owner Instance;                    // 0x1F08 (size: 0x8)
    TArray<FName> MPCStencilNameToResetOnDeath;                                       // 0x1F10 (size: 0x10)
    int32 XRayWantedCount;                                                            // 0x1F20 (size: 0x4)
    bool IsEnemyFirstPlayerPawn;                                                      // 0x1F24 (size: 0x1)
    int32 XRayBlockerCount;                                                           // 0x1F28 (size: 0x4)
    bool bIsCaptainEnabled;                                                           // 0x1F2C (size: 0x1)
    int32 StructureBrokenOccurences;                                                  // 0x1F30 (size: 0x4)
    FBP_AICharacter_Base_COnSlapstickFightDeactivate OnSlapstickFightDeactivate;      // 0x1F38 (size: 0x10)
    void OnSlapstickFightDeactivate();
    class UMaterialInterface* InvisibleAI_Material;                                   // 0x1F48 (size: 0x8)
    class UMaterialInstanceDynamic* InvisibleAI_MID;                                  // 0x1F50 (size: 0x8)
    bool bCanBeDoppelganger;                                                          // 0x1F58 (size: 0x1)
    FOutfitData ReplayOutfitData;                                                     // 0x1F60 (size: 0x88)
    bool bReplayOnInvisibleAI;                                                        // 0x1FE8 (size: 0x1)

    void ApplyInvisibleMaterial();
    void OnRep_ReplayIsInvisible();
    void OnRep_ReplayOutfitData();
    void SetupDopplegangers();
    void SetupInvisibleAI();
    void TriggerFadeOut_InvisbleAI();
    ...
    void m_OnCheatsUpdated_Event(bool _bApplied);
